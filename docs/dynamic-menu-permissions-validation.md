...
1) 当前用户权限
   - `GET /api/auth/me/resources`
     ...
     - 示例：
       ```json
       {"code":200,"data":{"resources":[{"id":1,"type":"MENU","code":"dashboard","name":"Dashboard","path":"/","component":"dashboard/Dashboard","icon":"dashboard","parentId":null,"orderNum":1,"meta":"{\"keepAlive\":true}"}],"actions":{"10":["create","export"]},"permissionVersion":3}}
       ```
...
2) 资源管理（Admin）
   ...
   - `POST /api/admin/resources` Body：
     ```json
     {"type":"MENU","code":"demo.menu","name":"演示","path":"/demo","component":"demo/Index","icon":"experiment","parentId":null,"orderNum":99,"propsJson":"{}","status":"enabled"}
     ```
