...
本系统将“员工（employee）”、“后台用户（sys_user）”与“平台账号（platformType/platformUserId）”统一成一条可追踪的闭环：

- 员工数据来源：组织同步（企微/钉钉/飞书）或手工导入；
- 后台用户：为员工创建登录主体（可与员工一对一绑定）；
- 平台账号：用于消息、扫码登录、组织同步标识，记录在员工与用户两端。

## 一、约束与不变量

- 同一平台 `platformUserId` 仅能绑定一个后台用户；
- 一个员工仅能绑定一个后台用户（`sys_user.employee_id`）；
- 平台字段双向补齐：
  - 若用户/员工一方已有 `platformType/platformUserId`，另一方为空则自动补齐；
...
- 组织同步后：
  - 对新增/更新员工调用 `ensureUserForEmployee(employee)`，若无对应用户则按规则创建：
    - `username`：优先 `employeeId`，否则手机、邮箱、或 `platformType_platformUserId`；
...
- 绑定平台账号：
  - `PUT /api/admin/users/{id}/platform-binding`（Body 含 `platformType`、`platformUserId`）
