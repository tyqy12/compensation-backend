...
-- RBAC tables are referenced by security filters and role services.
...
DROP TABLE IF EXISTS `salary_template_version`;
...
CREATE TABLE `pay_cycle` (
  ...
  `rule_template_id` bigint DEFAULT NULL COMMENT '绑定的薪资规则包ID',
  `rule_template_version` bigint DEFAULT NULL COMMENT '绑定的薪资规则包版本',
  ...
  KEY `idx_cycle_rule_template` (`rule_template_id`,`rule_template_version`)
...
-- ============================================
-- 5.1 薪资审批后置支付失败补偿表 (payroll_payment_failure)
-- ============================================
DROP TABLE IF EXISTS `payroll_payment_failure`;
...
-- ============================================
-- 6.2 外部平台身份绑定表 (external_identity)
-- ============================================
...
-- 插入基础角色和默认管理员用户
INSERT INTO `sys_role` (`code`, `name`, `description`, `role_type`, `sort_order`, `is_editable`, `icon`, `status`, `create_by`) VALUES
