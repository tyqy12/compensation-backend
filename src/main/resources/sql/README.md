...
- `migrations/*.sql`：正向增量迁移脚本，用于已有数据库的结构演进和数据修复；不得放置回滚脚本。
- `rollback/*.sql`：手工回滚脚本，仅在明确回滚窗口内按单个文件执行，不参与常规增量迁移。
- `seed/*.sql`：初始化/补充种子数据脚本。

## 执行原则

1. 任何已有业务数据的环境（开发共享库、测试库、预发、生产）禁止执行 `schema.sql` 与 `init_clean.sql`。
2. 已上线或长期运行环境只允许执行 `migrations/*.sql` 的正向增量变更。
3. 新增数据库变更时优先提交 migration，不在 `schema.sql` 上做“仅线上生效”的临时修补。
4. 回滚脚本必须放在 `rollback/`，并由上线负责人按文件名和变更单单独确认后执行。
